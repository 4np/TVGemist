...
class LiveChannelCollectionViewCell: UICollectionViewCell {
    ...
    static let nibName = "LiveChannelCollectionViewCell"
    static let reuseIdentifier = "LiveChannelCollectionViewCellIdentifier"
    ...
    func configure(withEPG epg: LiveEPG) {
        // program title
        nameLabel.text = epg.currentSchedule?.program.title.localizedCapitalized
        
        // next program
        if let nextSchedule = epg.nextSchedule {
        ...
        getImage(forEPG: epg, andSize: imageView.focusedFrameGuide.layoutFrame.size)
        
        // get the channel logo for the current program
        getLiveChannelLogoImage(forLiveChannel: epg.channel, andSize: logoImageView.focusedFrameGuide.layoutFrame.size)
    }
    ...
    private func getImage(forEPG epg: LiveEPG, andSize size: CGSize) {
        guard let program = epg.currentSchedule?.program else {
            getLiveChannelImage(forLiveChannel: epg.channel, andSize: size)
            return
        }
        
        getProgramImage(forProgram: program, channel: epg.channel, andSize: size)
    }
